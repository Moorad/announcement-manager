	...
	<div class="w-full">
		<form action="{{route('comments.store')}}" method="POST" id='comment-form'>
			...
			<div class='mx-auto w-full max-w-[40rem]'>
				...
				<textarea name="content" id="content" class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm h-28 resize-none"></textarea>
				<div class='flex justify-end'><button class='bg-blue-500 text-white px-5 py-2 rounded-md' onclick="event.preventDefault();submitComment(this)">Send</button></div>
				</div>
		</form>
